<?php
	...
	if (function_exists('register_sidebars')) {
		register_sidebar(array('id' => 'home-sidebar', 'name' => __('Home widgets', 'ncgbase'), 'description' => __('Shows on home page', 'ncgbase')));

		register_sidebar(array('id' => 'footer-sidebar', 'name' => __('Footer widgets', 'ncgbase'), 'description' => __('Shows in the sites footer', 'ncgbase')));
		register_sidebar(array('id' => 'sidebar', 'name' => __('Sidebar Widgets', 'ncgbase'), 'description' => __('Shows on the regular pages', 'ncgbase')));
	}
	...
	function ncgbase_enqueue_scripts() {
		...
		wp_enqueue_script('bootstrap', get_template_directory_uri() . '/bower_components/bootstrap-sass/dist/js/bootstrap.min.js', array(), '1.0', true);
	...
	function ncgbase_customize_register($wp_customize) {
		...
		$wp_customize -> add_section($section, array('title' => __('Homepage Goodies', 'ncgbase'), 'priority' => 30, 'panel' => wp_get_theme() . '-panel'));

		// Show map?
		$wp_customize -> add_setting('ncg_map_active', array('default' => 'false', 'transport' => 'refresh', 'sanitize_callback' => 'ncgbase_sanitize_layout'));

		$wp_customize -> add_control(new WP_Customize_Control($wp_customize, 'ncg_map_active', array('label' => __('Show map?', 'ncgbase'), 'section' => $section, 'settings' => 'ncg_map_active', 'type' => 'radio', 'choices' => array('true' => __('Show', 'ncgbase'), 'false' => __('Hide', 'ncgbase')))));
		// The embed code for maps
		$wp_customize -> add_setting('ncg_map_embed', array('default' => 'false', 'transport' => 'refresh', 'sanitize_callback' => 'ncgbase_sanitize_layout'));

		$wp_customize -> add_control(new WP_Customize_Control($wp_customize, 'ncg_map_embed', array('label' => __('Paste the embed code from google', 'ncgbase'), 'section' => $section, 'settings' => 'ncg_map_embed', 'type' => 'text')));
		// Slider
		$wp_customize -> add_setting('ncg_slider_query', array('default' => '', 'transport' => 'refresh'));

		$wp_customize -> add_control(new WP_Customize_Control($wp_customize, 'ncg_slider_query', array('label' => __('Post category for slider', 'ncgbase'), 'section' => $section, 'settings' => 'ncg_slider_query', 'type' => 'select', 'choices' => $cats)));
		// Slider
		$wp_customize -> add_setting('ncg_action', array('default' => '', 'transport' => 'refresh', 'sanitize_callback' => 'ncgbase_sanitize_layout'));

		$wp_customize -> add_control(new WP_Customize_Control($wp_customize, 'ncg_action', array('label' => __('Call to action items', 'ncgbase'), 'section' => $section, 'settings' => 'ncg_action', 'type' => 'select', 'choices' => $cats)));
	}
